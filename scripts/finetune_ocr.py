#!/usr/bin/env python3
r"""
EasyOCR japanese_g2 ファインチューニング実行スクリプト。
事前に以下を完了させること:
  1. git clone https://github.com/clovaai/deep-text-recognition-benchmark.git tools/deep-text-recognition-benchmark
  2. copy C:\Users\rotat\.EasyOCR\model\japanese_g2.pth pretrained\japanese_g2.pth
  3. venv\Scripts\python.exe -m pip install natsort tqdm
実行:
  venv\Scripts\python.exe scripts\finetune_ocr.py [--num_iter 5000] [--batch_size 32]
"""
import subprocess, sys, os, argparse

# japanese_g2 のcharacter セット（EasyOCR config.py より）
...
def main():
    ...
    if not os.path.exists(train_py):
        ...
        print(f"  期待パス: {train_py}")
        sys.exit(1)
    if not os.path.exists(pretrained):
        print("[ERROR] pretrained/japanese_g2.pth が見つかりません。")
        print(r"  copy C:\Users\rotat\.EasyOCR\model\japanese_g2.pth pretrained\japanese_g2.pth")
        sys.exit(1)

    cmd = [
        sys.executable, train_py,
        '--train_data',       os.path.join(project_root, 'data', 'ocr_lmdb', 'train'),
        '--valid_data',       os.path.join(project_root, 'data', 'ocr_lmdb', 'val'),
        ...
        '--valInterval',      str(args.val_interval),
        '--FT',
    ...
    print(f"  num_iter={args.num_iter}, batch_size={args.batch_size}")
    print(f"  保存先: saved_models/{args.exp_name}/")
