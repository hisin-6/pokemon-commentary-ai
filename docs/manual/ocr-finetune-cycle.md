...
**収集済み動画**: 全99本・12,730件（全件reviewed完了・2026-06-07時点）  
...
> **現行モデル**: v10（99.246%）で稼働中。`pretrained\pokemon_g2_v10.pth` にバックアップ済み。

### v10時点の残存誤読（v11優先収集対象）

| 誤読 | 正解 | 優先度 | 対象ROI |
|------|------|-------|---------|
| バッグンだ | バツグンだ | 最高 | msg |
| しろいハープ | しろいハーブ | 高 | ability_player/opp |
| ねっぶう | ねっぷう | 中 | msg |
| ダブルウイング | ダブルウィング | 中 | msg |
| ムーンフオース | ムーンフォース | 中 | msg |
