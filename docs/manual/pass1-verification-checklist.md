...
**2026-08-14〜: NG恒久対策フェーズ1を実装済み**（累計180件のNG傾向分析に基づき、
コスト対効果の高い4施策を先行実装。以降`screen_pass1.py`が下記をフラグしたら
「対策漏れの再発」を意味するので優先して原因を確認すること）:
- **施策A: プロンプト安全策強化** — 技の対象/防御結果/陣営が不明な場合に断定しない
  指示を追加（`kurepi_persona.py`/`server.py`）。ダブル対象取り違え・まもる無視/捏造・
  陣営取り違え全般への横断的な緩和策
- **施策B: 技効果ヒントRAG新設** — `data/pokedb.sqlite`にPokeAPI由来の技効果テキストを
  追加し、`type_hint`と同じ設計で実況プロンプトに注入（累計最頻NGパターン「技の効果に
  関する事実誤認」対策）。`screen_pass1.py`の**⑦変化技のダメージ誤認疑い**が再発検出用
- **施策C: is_opponent陣営判定クロスチェック** — 技ログの陣営誤検出（OCR文字列
  ヒューリスティックの弱さ）による陣営逆転・ロスター誤登録を防止。
  `screen_pass1.py`の**⑧陣営矛盾疑い**が再発検出用（簡易版・完全な検出ではない）
- **施策D: 紛らわしい技ペア対策** — OCR断片が複数の実在技名に僅差でマッチする場合
  （例: パワージェム⇔パワーシェア）にconfidentを降格し断定登録を防ぐ（`classifier.py`）
- フェーズ2（ダブル対象の画面座標ベース追跡・ターン境界イベント順序の再設計等）は
  今回スコープ外。詳細は`docs/daily/2026-08-14.md`参照
