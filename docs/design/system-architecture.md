...
ポケモン対戦画面を認識し、3Dモデル（VTuberアバター）が音声で実況するAIシステム。

> **開発方針（2026-07-13・[ADR-009](../adr/ADR-009-video-first-commentary.md)）**: 当面は録画済み対戦動画への実況付与（実況動画の制作）をターゲットとし、ライブ配信（リアルタイム実況）は動画での実況品質が確立した後に対応する。ライブモードのコードは残すが、ライブ特有の課題への改修は凍結中。
...
- **3秒以内**: イベント検知から音声再生まで3秒以内を目標とする（※ライブ配信時の目標。動画モードでは適用外・ADR-009）

---

## 2. コンポーネント図

⚠️ 以下はライブ配信モード（現在凍結中・上記ADR-009参照）の構成図で、全コンポーネントが
同時稼働する前提のVRAM注記になっている。動画ファースト運用（パス1解析→v2cアバター録画と
フェーズが分離）での実際のVRAM配分は[3. VRAM配分の詳細](#3-vram配分の詳細)を参照すること
（2026-08-04更新）。
...
⚠️ **2026-08-04更新**: 本セクションは策定時（2026-02、ライブ配信前提）の想定で、Phi-3 mini
（実況文生成）とバーチャルモーションキャプチャー（3Dモデル描画）が常時同時稼働する前提で
組まれていた。ADR-009（2026-07-13・動画ファースト方針）以降、パイプラインは**解析（パス1）と
アバター録画（v2c）でフェーズが分離**しており、この2つは同時稼働しない。また実況文生成は
Bedrock Claude Haikuが優先で、Phi-3 miniは呼び出し失敗時のフォールバックのみ
（ADR-003追記・ADR-001参照）。以下は実態に合わせた表。

### フェーズ別のVRAM実測目安

| フェーズ | 稼働コンポーネント | VRAM消費 |
|---------|-------------------|---------|
| パス1（解析・実況生成） | EasyOCR(0〜0.5GB) + YOLOv8(0.5〜1GB) + Bedrock(クラウド・VRAM消費なし) | **約1〜1.5 GB** |
| v2c（アバター録画） | バーチャルモーションキャプチャー（3Dモデル描画） | **約4〜5 GB** |
| フォールバック時のみ | Phi-3 mini 4bit / Ollama（Bedrock失敗時・既定でアイドル時は自動アンロード） | +2〜3 GB（一時的） |
| **上限（RTX 3080）** | | **10 GB** |

パス1実行中はVMCが動いていないため、**最大約8.5GBがローカルLLM用に未使用**（VMCと同時稼働
させない前提）。ローカルLLM移行（改善ロードマップ⑦）を検討する際はこちらを前提にすること。

### VRAM逼迫時の対応策

1. Phi-3 mini/ローカルLLMのコンテキスト長を短縮（VRAM削減）
2. バーチャルモーションキャプチャーの描画解像度・品質設定を下げる
3. EasyOCRのバッチサイズを1に固定する
4. それでも足りない場合は実況文生成をBedrock Claude Haikuに一本化する（既に既定の挙動）
...
```
[1] 入力ソース選択（--input 指定時は動画ファイル、省略時はカメラ）
    動画:   cv2.VideoCapture("path/to/video.mp4")（現在の主入力・ADR-009）
    カメラ: cv2.VideoCapture(3)（OBS仮想カメラ・デバイス番号3・ライブ用＝凍結中）
    間隔: 1秒（動画モードはフレームレートに従い全フレーム処理）
...
> **注**: 以下はライブ配信時の遅延実測。動画モード（実況動画ファースト方針・ADR-009）では処理時間は成果物の品質に影響しないため、この遅延は制約にならない。
...
```
src/capture/
├── screen_capture.py    # カメラ/動画キャプチャ（cv2.VideoCapture）・EasyOCR
└── yolo_detector.py     # YOLOv8による状態異常・ボール・終了画面検出
...
└── render_sink.py       # 実況動画レンダリング素材出力（ADR-009 パス1・--render-out）
```

**レンダリング素材出力モード（`--render-out <dir>`・ADR-009 パス1）**

動画モードで指定すると、実況音声を再生する代わりに WAV ファイルと
マニフェスト（`manifest.jsonl`・1行=1実況、`event_time`=イベント検知時点の動画内時刻）を
出力する。パス2（合成スクリプト）がこれを読んで元動画に実況トラックを合成する。
...
### 7-1. 既知のHP誤読・未更新ケース（仕様として許容）

| ケース | 原因 | 影響 |
|--------|------|------|
| ダメージアニメーション中のHP未更新 | HPバーが急速変化中は安定化フィルタが変化を弾く。OCRはダメージ表示フォーマット（`今ひとつのようだ / リキキリン / 6/211`）を検出するが、構造化パーサーはコマンド選択画面フォーマットのみ対応のため無視する | 直後のターン開始時まで旧HPが残る。faint時はHP=0として正しく記録されるため実況精度への影響は軽微 |
| 連続ダメージ（例: ワイドフォース→ボディプレス）での中間HP消失 | 1発目ダメージ後のHP（例: 6/211）が2発目で即座にひんしになるため、安定化するまでの時間がない | 中間HP値がトラッカーに記録されないが、ひんし時のHP=0は正常記録される |
| HPpxのアニメーション中ノイズ | HP増加方向の変化（回復アニメーション等）は連続N秒の確認を要求するため、即時反映されない | 一時的なHP増加誤読が実況文に混入する可能性あり |
...
- [ADR-009](../adr/ADR-009-video-first-commentary.md) - 実況動画ファースト方針（ライブ配信は後段）
