...
> **注（2026-07-13・ADR-009）**: 実況動画ファースト方針への変更により、以下の遅延目標は動画モードでは**適用外**（処理時間は成果物の品質に影響しない）。ライブ配信再開時の目標として据え置く。
...
## 追記（2026-07-13）: 実況動画ファースト方針（ADR-009）

当面の開発ターゲットが録画済み動画への実況付与に変更された（[ADR-009](ADR-009-video-first-commentary.md)）。イベント駆動の検知・蓄積・生成の流れ自体は動画モードでも同一のため本ADRの決定に変更はないが、遅延目標は動画モードでは適用外となる（上記注参照）。ライブモード特有の課題（実況処理ブロック中のフレーム喪失）への対応はライブ再開時まで凍結。
