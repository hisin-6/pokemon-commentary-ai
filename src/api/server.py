...
def _build_vision_prompt(context: dict, history: list[str], battle_state: dict,
    ...
    # 交代の蒸し返し対策（2026-08-29新設）: switch_focusで触れられていない陣営
    # （自分側/相手側のどちらか、または両方）について、前のターンで既に起きた
    # 交代を「今起きたこと」として語らないよう明示する。switch_focus自体の
    # 有無に関わらずswitch/move_usedイベント全体に付与する（実機
    # 2026-08-28_22-11-26: switch_focus="自分のエルフーン"のみのmove_usedで、
    # 既に別イベントで報告済みの相手の交代を誤った起点付きで蒸し返した）。
    if event_type in ("switch", "move_used") and context.get("no_new_switch_hint"):
        event_hint += "。" + context["no_new_switch_hint"]
    ...
    lines = [
        ...
        *_SLANG_GLOSSARY_LINES,
        ...
        "- 「2回連続」「○ターン目」等の回数・連続性を表す数字は、下記の情報（技ログ・"
        "ターン数）から明確に数えられる場合のみ使う。技ログに1件しか記録が無いのに"
        "「2回連続」等と勝手に数を盛らないこと",
...
def _build_script_prompt(gap: dict, events: list, moments: list = None,
    ...
    lines += [
        ...
        *(
            ...
            if mode == "predict"
                ...
                "- 直前の展開の振り返り・戦況の整理（HP・残り頭数）。ただし直前のイベント実況が",
                "  既に伝えた出来事（「○○が落ちた」「○○が効果抜群だった」等）を同じ情報量で",
                "  なぞるのではなく、そこからどう繋がるか（次にどうすべきか・残りの展開への",
                "  影響等）に重心を置くこと",
                "- タイプ相性や特性をふまえた考察",
                "- 次の展開の予想",
                "",
    ...
    lines += [
        ...
        "※ 収録済み実況と重複しない内容にすること。特に「○○が場を離れた」",
        "「○○が効果抜群だった」等、直前のイベント実況（下記の[event_type]付きの行）が",
        "既に伝えた出来事の事実を、同じ情報量のまま言い直さないこと。その出来事に触れる",
        "場合は、必ず新しい話題（そこからの展開・戦況の変化・考察等）につなげること。",
    ]
...
def _build_payoff_prompt(prediction_text: str, hit: bool, outcome_summary: str,
    ...
    2026-08-29修正: outcome_summaryには決定的イベント（battle_end/faint）の
    実況文そのものが渡ることがある（generate_predictions.pyのdecisive.get(
    "commentary")）。この内容をLLMがそのまま言い直してしまい、既存の
    battle_end等の実況と内容が重複する事故があった（実機2026-08-28_22-11-26:
    battle_endの「降参が選ばれ、こちらの勝利だ」をprediction_payoffが
    「ついに相手が降参！」とほぼ同じ内容で9.8秒後に繰り返した）。
    outcome_summaryは「状況の背景説明」として渡すに留め、その内容を
    繰り返さず反応に徹するよう明示する。
    """
    if persona == "neutral":
        # 2026-08-29: _NEUTRAL_CHARACTER_INTRO_LINES（_build_vision_prompt用）とは別に
        # コピペでハードコードされていたため、先日の口調修正（ですます調偏重の解消）が
        # ここには反映されていなかった（実機2026-08-28_22-11-26のselection_predictionで
        # ユーザー発見）。同じ指示を追加して揃える。
        intro_lines = [
            "あなたは、ポケモン対戦実況AIVTuberです。",
            "テンション高めだが落ち着いた、中立的な実況口調で話してください"
    ...
    lines = [
        *intro_lines,
        ...
        "- 「その後の展開」に書かれている出来事（何が起きたか）は既に別の実況で"
        "伝え済みなので、同じ内容をそのまま言い直さないこと。あなたの予想が"
        "当たった/外れたことへの感想・反応そのものに焦点を絞ること",
        "- 鉤括弧（「」）は使わない",
