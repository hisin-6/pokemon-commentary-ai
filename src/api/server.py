...
# 丁寧な言い切りへの偏りを避け、タメ口寄りの言い切りも積極的に混ぜる指示を追加
# （キャラクターとしての名乗り・自称はneutral運用の方針のまま引き続き禁止）。
_NEUTRAL_CHARACTER_INTRO_LINES = [
    "あなたは、ポケモン対戦実況AIVTuberです。",
    "テンション高めだが落ち着いた、中立的な実況口調で話してください"
    "（キャラクターとしての自己紹介・名乗り・一人称のキャラ付けはしないこと）。",
    "「〜です」「〜ます」の丁寧な言い切りだけに偏らず、「〜だ」「〜だね」「〜だよ」のような"
    "話し言葉・タメ口寄りの言い切りも積極的に混ぜて、実況らしい勢いと自然さを出すこと"
    "（毎回同じ文末パターンで終わらせないこと）。",
...
# 2026-08-29: 全例文が「ですます調」に偏っていたため、タメ口寄りの言い切りと
# 混在させた例文に更新（ユーザーfb「もっと喋り口調に・ですます調を少なく」対応）。
_NEUTRAL_TONE_EXAMPLES: list[tuple[str, str]] = [
    ("自分の場の2匹が、相手の1匹に技を集中させた",
     "狙うは1匹！2匹がかりの集中攻撃！これは決まってほしいね"),
    ("自分のポケモンがつるぎのまいで能力を上げた",
     "ここで積んできた！攻撃力アップで一気に展開のチャンスが広がった"),
    ("相手の範囲技が自分の場の2匹に着弾した",
     "範囲技が両方に直撃だ！でもまだ大丈夫、ここからしっかり受けていくよ"),
]
...
def _build_vision_prompt(context: dict, history: list[str], battle_state: dict,
    ...
    if event_type == "move_single":
        ...
        # 2026-08-28〜: 通常のfaintイベント（画面にHP=0表示が映っている）でも
        # fainted_names()のdiffで確定した対象を同じフィールドで併せて渡すようになった
        # （実機2026-08-28_21-52-34でダブルバトルの同時ダウンをLLMが誤った種族名で
        # 実況した反証あり）。両ケースで使うため「画面に映っていない」と断定する文言は外す。
        event_hint = (
            f"「{context.get('faint_focus', '')}」が倒れたことが蓄積した戦況データから確定した"
            "（画面にHP=0表示や「たおれた」等が見えている場合はそれとも一致する、信頼度の高い情報）。"
            "このポケモンが倒れたことだけを実況する（他のポケモンの名前を推測で使わないこと）"
        )
    ...
    elif event_type == "move_used" and context.get("switch_focus"):
        # 交代ヒント（move_used版・2026-08-27新設）: move_usedもswitchと同じく
        # 「コマンドが確定した瞬間」に発火し、交代コマンドを選んだターンでは実際の
        # 繰り出しが少し後（ディスパッチ時点ではまだ画面外）になる。switchイベント
        # には専用の強い指示（上記）があるが、move_usedは従来「過去の交代を今起きた
        # かのように語らない」という否定形の注意書きしか無く、switch_focusが戦況
        # テキストの中の一行として埋もれて無視される事故があった（実機
        # renders/2026-08-26_21-35-16の298.4s: switch_focusは「自分のイダイトウ」
        # なのに、まだ場に残っていた古い相手情報から「相手がスコヴィランへ交代して
        # きました」と無関係な内容を実況していた）。switchと同様に断定・確定した
        # 事実として直接指示する。
        event_hint = (
            "コマンドが確定して新しいターンの攻防が始まる場面。このターン、画面の"
            f"繰り出しメッセージから実際に場に出た（出る）ポケモンは「{context.get('switch_focus', '')}」"
            "であることが確定している（信頼度高）。戦況情報に残っている別のポケモンの"
            "名前や、それより前のターンの交代を、今回の交代として語らないこと。"
            "交代以外の戦況全体（HP状況・残り数・有利不利）にも触れてよい"
        )
    elif event_type == "mega_evolution" and context.get("mega_evolution_focus"):
        # メガシンカの専用変身アニメーション（2026-08-29新設）: 数秒間の専用演出中は
        # 他の技・戦況の動きが発生しないため、従来この区間が無実況になっていた
        # （ユーザー指摘）。move_singleと同様、この1つの出来事だけに焦点を絞らせる。
        # 将来Zワザ/ダイマックス/テラスタルを追加する際も同じ形の分岐を想定
        # （画面文言未確認のため今回は見送り）。
        event_hint = (
            f"「{context.get('mega_evolution_focus', '')}」がメガシンカする専用の変身"
    ...
    lines += [
        ...
        # 2026-08-27: ユーザーfb「イダイトウが舞台に戻るってなに？　控えに戻るなら
        # わかりますが」（renders/2026-08-26_21-35-16）。ポケモン対戦に「舞台」という
        # 言葉は存在しないため、交代で場に出る表現として不自然だった。
        "- ポケモンが場に出る/戻ることを「舞台に戻る」「舞台に上がる」等の演劇的な"
        "比喩で表現しないこと（「場に出る」「繰り出す」「登場する」等、対戦用語として"
        "自然な表現を使う）",
